...
class ListHandle extends CollectionHandleBase<realm_list> implements intf.ListHandle {
  ...
  @override
  bool get isValid => realm_list_is_valid(pointer);

  @override
  ResultsHandle asResults() {
    return ResultsHandle(realm_list_to_results(pointer), root);
  }
  ...
  int get size {
    return using((arena) {
      ...
      realm_list_size(pointer, size).raiseLastErrorIfFalse();
      return size.value;
    });
  }
  ...
  void removeAt(int index) {
    realm_list_erase(pointer, index).raiseLastErrorIfFalse();
  }

  @override
  void move(int from, int to) {
    realm_list_move(pointer, from, to).raiseLastErrorIfFalse();
  }

  @override
  void deleteAll() {
    realm_list_remove_all(pointer).raiseLastErrorIfFalse();
  }
  ...
  int indexOf(Object? value) {
    return using((arena) {
      ...
      realm_list_find(pointer, realmValue, outIndex, outFound).raiseLastErrorIfFalse();
  ...
  @override
  void clear() {
    realm_list_clear(pointer).raiseLastErrorIfFalse();
  }
  ...
  Object? elementAt(Realm realm, int index) {
    return using((arena) {
      ...
      realm_list_get(pointer, index, realmValue).raiseLastErrorIfFalse();
      return realmValue.toDartValue(realm, () => realm_list_get_list(pointer, index), () => realm_list_get_dictionary(pointer, index));
    });
  }
  ...
  ListHandle? resolveIn(covariant RealmHandle frozenRealm) {
    return using((arena) {
      ...
      realm_list_resolve_in(pointer, frozenRealm.pointer, resultPtr).raiseLastErrorIfFalse();
  ...
  void addOrUpdateAt(int index, Object? value, bool insert) {
    using((arena) {
      final realmValue = value.toNative(arena);
      (insert ? realm_list_insert : realm_list_set)(pointer, index, realmValue.ref).raiseLastErrorIfFalse();
    });
  }
  ...
  void addOrUpdateCollectionAt(Realm realm, int index, RealmValue value, bool insert) {
    createCollection(
      realm,
      value,
      () => (insert ? realm_list_insert_list : realm_list_set_list)(pointer, index),
      () => (insert ? realm_list_insert_dictionary : realm_list_set_dictionary)(pointer, index),
    );
  }

  @override
  ObjectHandle setEmbeddedAt(int index) {
    return ObjectHandle(realm_list_set_embedded(pointer, index), root);
  }

  @override
  ObjectHandle insertEmbeddedAt(int index) {
    return ObjectHandle(realm_list_insert_embedded(pointer, index), root);
  }
  ...
  ResultsHandle query(String query, List<Object?> args) {
    return using((arena) {
      ...
      final queryHandle = QueryHandle(realm_query_parse_for_list(pointer, query.toCharPtr(arena), length, argsPointer), root);
