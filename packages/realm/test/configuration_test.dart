...
void main() {
  ...
  test('Configuration.initialDataCallback not invoked for existing realm', () {
    ...
    final configAgain = Configuration.local(
      [Person.schema],
      initialDataCallback: (realm) {
        invokedAgain = true;
        realm.add(Person('p1'));
  ...
  test('Configuration.initialDataCallback is a no-op, '
      'when opening an empty existing Realm', () {
  ...
  for (var shouldCompact in [true, false]) {
    test('Configuration.shouldCompact, '
        'when return $shouldCompact triggers compaction', () async {
      var config = Configuration.local([Person.schema]);
  ...
  test('Configuration.local set too short encryption key size', () {
    List<int> key = [1, 2, 3];
    expect(() => Configuration.local([Car.schema], encryptionKey: key), throws<RealmException>("Wrong encryption key size"));
  });
  ...
  test('LocalConfiguration.maxNumberOfActiveVersions - freeze to exceed the version and, '
      'then throw', () {
