...
import 'package:realm/src/logging.dart';
import 'package:realm/src/handles/realm_core.dart';
...
void main() {
  ...
  group('All levels', () {
    for (var level in LogLevel.values) {
      test('Realm.logger supports log level $level', () {
        ...
        final tag = Uuid.v4();
        expectLater(Realm.logger.onRecord, emits((category: LogCategory.realm.sdk, level: level, message: '$level $tag')));
  ...
  test('RealmLogger hookup logging', () async {
    ...
    expectLater(
      logger.onRecord,
      emits(isA<DartLogRecord>().having((r) => r.level, 'level', Level.SEVERE).having((r) => r.message, 'message', 'error')),
