...
void main() {
  ...
  test('Results sort', () {
    ...
    realm.write(() => realm.addAll([Person("Michael"), Person("Sebastian"), Person("Kimi")]));
  ...
  test('query by condition on decimal128', () {
    ...
    realm.write(() => realm.addAll([ObjectWithDecimal(small), ObjectWithDecimal(big)]));
  ...
  test('Query with list, sets and iterable arguments', () {
    final config = Configuration.local([Person.schema]);
    Realm realm = getRealm(config);
    realm.write(() => realm.addAll([Person('Ani'), Person('Teddy'), Person('Poly')]));
  ...
  test('RealmResults.skip', () {
    ...
    for (var i = 0; i < max; i++) {
      ...
      expect(() => results.skip(max + i + 1), throwsA(isA<RangeError>().having((e) => e.invalidValue, 'count', max + i + 1)));
      expect(() => results.skip(-i - 1), throwsA(isA<RangeError>().having((e) => e.invalidValue, 'count', -i - 1)));
    }
  });
