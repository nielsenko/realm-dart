//! OpenSSL 3.3.2 libcrypto source manifest for the no-asm in-module build
//! used on Android: zig bundles no bionic libc, so the openssl package's own
//! artifact cannot be built for Android targets - instead the hook-wired
//! module (which has the NDK libc) compiles the same sources directly.
//!
//! Extracted from the allyourcodebase/openssl 3.3.2 recipe build.zig:
//! its crypto + providers groups, minus the ssl group and the x86_64
//! assembly overlay (the C files carry portable fallbacks for every arch).
//! `.upstream` paths live in the openssl_src package, `.recipe` paths in
//! the openssl (recipe) package.

pub const Root = enum { upstream, recipe };
...
pub const groups = [_]Group{
    .{ .root = .upstream, .prefix = "ssl", .files = &upstream_ssl_cbc },
    .{ .root = .upstream, .prefix = "providers", .files = &upstream_providers_0 },
    .{ .root = .recipe, .prefix = "providers", .files = &recipe_providers_1 },
    .{ .root = .upstream, .prefix = "crypto", .files = &upstream_crypto_2 },
    .{ .root = .recipe, .prefix = "crypto", .files = &recipe_crypto_3 },
};

const upstream_providers_0 = [_][]const u8{
    "baseprov.c",
    "common/bio_prov.c",
...
const recipe_providers_1 = [_][]const u8{
...
const upstream_crypto_2 = [_][]const u8{
    "aes/aes_cbc.c",
    "aes/aes_core.c",
    "aes/aes_cfb.c",
    "aes/aes_ecb.c",
...
const recipe_crypto_3 = [_][]const u8{
    "params_idx.c",
};


// Not the ssl library: just the constant-time CBC/MAC helpers that
// libcrypto's AES-CBC-HMAC cipher implementations link against.
const upstream_ssl_cbc = [_][]const u8{
