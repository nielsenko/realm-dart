...
class SetHandle extends RootedHandleBase<realm_set> implements intf.SetHandle {
  ...
  ResultsHandle get asResults {
    return ResultsHandle(realm_set_to_results(pointer), root);
  }
  ...
  ResultsHandle query(String query, List<Object?> args) {
    return using((arena) {
      ...
      final queryHandle = QueryHandle(realm_query_parse_for_set(pointer, query.toCharPtr(arena), length, argsPointer), root);
  ...
  bool insert(Object? value) {
    return using((arena) {
      ...
      realm_set_insert(pointer, realmValue.ref, outIndex, outInserted).raiseLastErrorIfFalse();
  ...
  Object? elementAt(Realm realm, int index) {
    return using((arena) {
      ...
      realm_set_get(pointer, index, realmValue).raiseLastErrorIfFalse();
  ...
  bool find(Object? value) {
    return using((arena) {
      ...
      realm_set_find(pointer, realmValue.ref, outIndex, outFound).raiseLastErrorIfFalse();
  ...
  bool remove(Object? value) {
    return using((arena) {
      ...
      realm_set_erase(pointer, realmValue.ref, outErased).raiseLastErrorIfFalse();
  ...
  @override
  void clear() {
    realm_set_clear(pointer).raiseLastErrorIfFalse();
  }
  ...
  int get size {
    return using((arena) {
      ...
      realm_set_size(pointer, outSize).raiseLastErrorIfFalse();
  ...
  @override
  bool get isValid {
    return realm_set_is_valid(pointer);
  }

  @override
  void deleteAll() {
    realm_set_remove_all(pointer).raiseLastErrorIfFalse();
  }
  ...
  SetHandle? resolveIn(covariant RealmHandle frozenRealm) {
    return using((arena) {
      ...
      realm_set_resolve_in(pointer, frozenRealm.pointer, resultPtr).raiseLastErrorIfFalse();
